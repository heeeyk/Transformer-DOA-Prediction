...
from model.base_transformer.meta_models.merging_model import MergingModel
from model.base_transformer.transformer_xl.lower_upper_config import activation_dict
...
class MyTransformer(torch.nn.Module):
    def __init__(
        ...
        self.mask = generate_square_subsequent_mask(seq_length).cuda()
        self.dense_shape = torch.nn.Linear(number_time_series, d_model)
        self.dense_t = torch.nn.Linear(6, d_model)
        ...
        self.tgt_mask = generate_square_subsequent_mask(seq_length).cuda()
        self.sigmoid = None
        if sigmoid:
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, tgt_mask=None, src_mask=None):
        ...
        return self.decode_seq(x, x, tgt_mask)
    ...
    def encode_sequence(self, x, src_mask=None):
        ...
        # x = self.my_encoder(src=x, mask=src_mask)
        # x = self.my_encoderlayer(src=x)
        return x
    ...
    def decode_seq(self, mem, t, tgt_mask=None, view_number=None) -> torch.Tensor:
        ...
        # t = self.basic_t(t)
        # print(mem.shape, t.shape, tgt_mask.shape)
        x = self.transformer.decoder(t, mem, tgt_mask=tgt_mask)
        # x = self.my_decoder(t, mem, tgt_mask=tgt_mask)
        x = self.final_layer(x).squeeze()
        # print(x)
        # x = torch.mean(x, dim=0)
        if self.sigmoid:
            x = self.sigmoid(x)
        # return x[-1].view(-1, view_number)
        return x
...
class SimpleTransformer(torch.nn.Module):
    def __init__(
        ...
        self.my_encoderlayer = TransformerEncoderLayer(d_model=d_model, nhead=n_heads)
        self.my_encoder = TransformerEncoder(encoder_layer=self.my_encoderlayer, num_layers=6, norm=None)

        self.final_layer = torch.nn.Linear(d_model, 1)
        # self.out_layer = torch.nn.Sequential(torch.nn.Linear(180, 32),
        #                                      torch.nn.Linear(32, 1))
        self.sequence_size = seq_length
        self.tgt_mask = generate_square_subsequent_mask(seq_length)
        self.sigmoid = None
        if sigmoid:
            self.sigmoid = torch.nn.Sigmoid()
    ...
    def decode_seq(self, mem, t, tgt_mask=None, view_number=None) -> torch.Tensor:
        ...
        # print(mem.shape, t.shape, tgt_mask.shape)
        # x = self.transformer.decoder(t, mem, tgt_mask=tgt_mask)
        x = self.transformer.decoder(t, mem)
        x = self.final_layer(x).squeeze()
        x = x[-1]
        if self.sigmoid:
            x = self.sigmoid(x)
...
def greedy_decode(
    ...
    for i in range(max_len):
        mask = generate_square_subsequent_mask(i + 1).cuda()
        with torch.no_grad():
