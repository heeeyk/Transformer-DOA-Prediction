...
if __name__ == "__main__":
    ...
    """
    print('pk-pd')
    pkpd_out = d_box.ceload(case_nums=test_patch)
...
for i in tqdm.tqdm(range(27, 30)):
    # plt.figure()
    plt.rcParams['figure.figsize'] = (6.0, 8.0)
    plt.subplot(2, 1, 1)
    ...
    plt.ylabel("Bispectral index")
    plt.subplot(2, 1, 2)
    plt.plot(new[1][i], label='Baseline', color='darkorange')
    plt.ylabel("Bispectral index")
    plt.xlabel("Time(sec)")
    plt.legend(loc='upper right')
    plt.show()
    # plt.savefig(f'/HDD_data/HYK/bis/output/compare0/case{i}.png')
    plt.close()
...
all = [15, 5, 27, 29, 21, 46, 47, 15, 72]
axis_range = [11000, 7500, 13000, 9500, 8500, 11000, 12000, 11000, 6500]
fig = plt.figure(figsize=(24, 16))
color_i = [
    plt.Rectangle((0, 0), 1000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1400, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 2000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1100, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1000, 100, color="paleturquoise"),
    plt.Rectangle((0, 0), 1500, 100, color="paleturquoise")]
color_m = [
    plt.Rectangle((1000, 0), 7500, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 5000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 9000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 7000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 6000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 8000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 9500, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 8000, 100, color="mistyrose"),
    plt.Rectangle((1000, 0), 4500, 100, color="mistyrose")]
color_r = [
    plt.Rectangle((8000, 0), 3000, 100, color="cornsilk"),
    plt.Rectangle((6000, 0), 2000, 100, color="cornsilk"),
    plt.Rectangle((10000, 0), 3000, 100, color="cornsilk"),
    plt.Rectangle((8000, 0), 1500, 100, color="cornsilk"),
    plt.Rectangle((7000, 0), 1500, 100, color="cornsilk"),
    plt.Rectangle((9000, 0), 2000, 100, color="cornsilk"),
    plt.Rectangle((10500, 0), 1500, 100, color="cornsilk"),
    plt.Rectangle((9000, 0), 2000, 100, color="cornsilk"),
    plt.Rectangle((5500, 0), 1000, 100, color="cornsilk")]

ax = 0
for i, case in enumerate(all):
    ax = fig.add_subplot(331+i)
    ax.add_patch(color_r[i])
    ax.add_patch(color_m[i])
    ax.add_patch(color_i[i])

    ax.plot(test_label[case], 'silver', label='Ground True')
    ax.plot(new[1][case], label='Baseline', color='lightcoral')
    plt.plot(pkpd_out[case], label='PK-PD', color='green')
    ax.plot(new[0][case], label='Ours')

    plt.ylabel("Bispectral index", fontsize=12)
    plt.xlabel("Time(sec)", fontsize=12)
    plt.xlim(0, axis_range[i])
    plt.ylim(0, 100)
ax.legend(['Recovery', 'Maintenance', 'Induction', 'Ground True', 'Baseline', 'Ours'],
          bbox_to_anchor=(0, 3.5), loc=3, borderaxespad=0, prop={'size': 20}, ncol=2)
plt.show()
plt.savefig(f'/HDD_data/HYK/bis/output/best_compare_1.png')
plt.close()
